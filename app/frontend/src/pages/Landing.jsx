...
export const Landing = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans flex flex-col selection:bg-indigo-100">
      ...
      {/* Hero Section */}
      <main className="pt-32 pb-20 flex-1 overflow-hidden">
        <div className="max-w-7xl mx-auto px-8">
          <div className="text-center space-y-6 mb-20 animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-6">
               <Button onClick={() => navigate('/login')} className="w-full sm:w-auto px-24 py-4 rounded-2xl text-lg font-black shadow-xl shadow-indigo-100 group transition-all hover:scale-105 active:scale-95 flex items-center justify-center">
                 Hemen Başla 
                 <ArrowRight className="ml-3 group-hover:translate-x-2 transition-transform" />
               </Button>
               <button onClick={() => navigate('/club-login')} className="w-full sm:w-auto px-10 py-4 bg-white border border-gray-200 text-gray-700 rounded-2xl text-lg font-black hover:bg-gray-50 hover:border-gray-300 transition-all">
                 Kulüp Girişi
               </button>
            </div>
          </div>

          {/* Dynamic Content Sections - Stacked with Horizontal Scroll */}
          <div className="space-y-20">
            
            {/* Top: Active Clubs (Horizontal Scroll) */}
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-black text-gray-900 tracking-tight flex items-center gap-2">
                  <Users className="text-indigo-600" size={24} /> Aktif Kulüpler
                </h2>
                <span className="px-3 py-1 bg-white border border-gray-100 text-gray-500 rounded-lg text-xs font-bold shadow-sm">
                  {clubs.length} Toplam
                </span>
              </div>

              <div className="flex flex-nowrap overflow-x-auto gap-6 pb-6 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide snap-x">
                {loading ? (
                  Array(4).fill(0).map((_, i) => (
                    <div key={i} className="min-w-[300px] h-24 bg-gray-200 animate-pulse rounded-[2rem]" />
                  ))
                ) : clubs.length === 0 ? (
                  <div className="w-full bg-white p-8 rounded-3xl border border-dashed border-gray-200 text-center">
                    <p className="text-gray-400 font-medium italic">Henüz onaylı kulüp yok.</p>
                  </div>
                ) : (
                  clubs.map(club => (
                    <div key={club.id} className="min-w-[300px] snap-start group bg-white border border-gray-100 p-5 rounded-[2rem] hover:border-indigo-500 hover:shadow-xl hover:shadow-indigo-50/50 transition-all duration-300 cursor-default">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 font-black text-xl group-hover:scale-110 transition-transform flex-shrink-0">
                          {club.logoUrl ? (
                        ...
                        <button onClick={() => navigate('/login')} className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:bg-indigo-600 hover:text-white transition-all flex-shrink-0 shadow-sm">
            ...
            {/* Bottom: Upcoming Events (Horizontal Scroll) */}
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-black text-gray-900 tracking-tight flex items-center gap-2">
              ...
              <div className="flex flex-nowrap overflow-x-auto gap-8 pb-10 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide snap-x">
                {loading ? (
                  Array(3).fill(0).map((_, i) => (
                    <div key={i} className="min-w-[380px] h-64 bg-gray-200 animate-pulse rounded-[2.5rem]" />
                  ))
                ) : events.length === 0 ? (
                  <div className="w-full bg-white p-20 rounded-[3rem] border border-dashed border-gray-200 text-center">
                    <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6 text-gray-300">
                      <Calendar size={40} />
                    </div>
                    <h3 className="text-xl font-bold text-gray-800">Henüz Etkinlik Yok</h3>
                    <p className="text-gray-400 mt-2">Şu an planlanmış bir etkinlik bulunmuyor.</p>
                  </div>
                ) : (
                  events.map(event => {
                    ...
                    return (
                      <div key={event.id} className="min-w-[380px] snap-start group relative bg-white border border-gray-100 rounded-[2.5rem] p-8 hover:shadow-2xl hover:shadow-indigo-100/50 transition-all duration-500 overflow-hidden flex flex-col">
                        <div className="absolute top-0 right-0 p-8">
                          <div className="w-14 h-14 bg-gray-50 rounded-2xl flex flex-col items-center justify-center border border-gray-100 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-500 shadow-sm">
                        ...
                        <h3 className="text-2xl font-black text-gray-900 mb-6 group-hover:text-indigo-600 transition-colors leading-tight min-h-[4rem] line-clamp-2">
                          {event.name}
                        </h3>
                        ...
                        <button onClick={() => navigate('/login')} className="w-full py-4 bg-gray-50 group-hover:bg-indigo-600 text-gray-600 group-hover:text-white rounded-2xl text-sm font-black transition-all duration-500 flex items-center justify-center gap-2 shadow-sm">
                          Detayları Gör & Katıl
                          <ArrowRight size={18} />
                        </button>
                      </div>
