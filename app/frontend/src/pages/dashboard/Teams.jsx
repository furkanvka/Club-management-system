...
export const Teams = () => {
  ...
  useEffect(() => { fetchTeams(); }, [activeClub]);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      alert('Ekip oluşturulamadı: ' + (e.response?.data || e.message));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{activeClub?.name} — {teams.length} ekip</p>
        </div>
        {canManage && (
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4 max-w-md">
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase">Ekip Adı</label>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-16 text-center">
          ...
          <p className="text-gray-400 font-medium">Henüz ekip yok.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {teams.map(t => (
            <div key={t.id} className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md transition group">
